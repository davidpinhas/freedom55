...
class TestArgo(unittest.TestCase):
    ...
    def test2_argo_create_app(self):
        ...
        runner = CliRunner()
        result = runner.invoke(argocd.create_app, ['--file', 'tmp_argo_app.json'])
        assert result.exit_code == 0
    ...
    def test3_argo_update_app(self):
        ...
        runner = CliRunner()
        result = runner.invoke(argocd.update_app, ['--file', 'tmp_argo_app.json'])
        assert result.exit_code == 0
    ...
    def test4_argo_delete_app(self):
        ...
        runner = CliRunner()
        result = runner.invoke(argocd.delete_app, ['--name', 'freedom55_argo_test_app'])
        assert result.exit_code == 0
